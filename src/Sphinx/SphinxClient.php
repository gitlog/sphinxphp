...
class SphinxClient
{
    ...
    public $_mode;         ///< query matching mode (default is self::SPH_MATCH_EXTENDED2)
    public $_weights;      ///< per-field weights (default is 1 for all fields)
    public $_sort;         ///< match sorting mode (default is self::SPH_SORT_RELEVANCE)
    ...
    public $_ranker;       ///< ranking mode (default is self::SPH_RANK_PROXIMITY_BM25)
    public $_rankexpr;     ///< ranking mode expression (for self::SPH_RANK_EXPR)
    ...
    public $_query_flags; ///< per-query various flags
    public $_predictedtime; ///< per-query max_predicted_time
    public $_outerorderby; ///< outer match sort by
    public $_outeroffset; ///< outer offset
    public $_outerlimit; ///< outer limit
    public $_hasouter;
    ...
    function __destruct()
    {
        if ($this->socket && $this->_socket !== false )
            fclose ( $this->_socket );
    }
        ...
        foreach ( $this->_overrides as $key => $entry )
        {
            ...
            foreach ( $entry["values"] as $id=>$val )
            {
                ...
                switch ( $entry["type"] )
                {
                    ...
                    case self::SPH_ATTR_BIGINT:   $req .= sphPackI64 ( $val ); break;
